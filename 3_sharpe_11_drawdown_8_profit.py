...
class Strategy:
    ...
    def __init__(self, rsi_period=14, rsi_overbought=70, rsi_oversold=30, atr_period=14, 
                 atr_multiplier=2.0, risk_pct=1.0, adx_period=14, adx_threshold=20, 
                 ema_short=20, ema_long=50, atr_threshold_pct=0.3, bb_period=20, bb_std=2.0,
                 chop_min_conditions=4, ema_slope_threshold=0.1, bb_width_threshold=3.5):
        """
        ...
        Parameters:
            ...
            adx_threshold (float): ADX threshold for trend strength (default: 20)
            ema_short (int): Period for short EMA calculation (default: 20)
            ema_long (int): Period for long EMA calculation (default: 50)
            atr_threshold_pct (float): Minimum ATR as percentage of price for trading (default: 0.3%)
            ...
            chop_min_conditions (int): Minimum number of conditions required to identify choppy market (default: 4)
            ema_slope_threshold (float): Maximum slope magnitude for EMAs to be considered flat (default: 0.1)
            bb_width_threshold (float): Threshold for Bollinger Band width below which market is considered range-bound (default: 3.5)
        """
        ...
        # New parameters for fine-tuning choppy market detection
        self.chop_min_conditions = chop_min_conditions  # Minimum conditions for choppy market
        self.ema_slope_threshold = ema_slope_threshold  # Threshold for flat EMAs
        self.bb_width_threshold = bb_width_threshold    # Threshold for tight Bollinger Bands
    ...
    def detect_choppy_market(self, df):
        ...
        flat_emas = (abs(df["ema_short_slope"]) < self.ema_slope_threshold) & (abs(df["ema_long_slope"]) < self.ema_slope_threshold)
        ...
        tight_bands = df["bb_width"] < self.bb_width_threshold
        
        # Calculate higher timeframe trend direction and strength
        df["ema_trend"] = (df["ema_short"] > df["ema_long"]).astype(int) * 2 - 1  # +1 for uptrend, -1 for downtrend
        df["trend_strength"] = abs(df["ema_short"] - df["ema_long"]) / df["close"] * 100
        
        # Create "choppiness index" using the Choppiness Index formula (0-100)
        # Higher values indicate choppier markets
        n = 14  # lookback period
        if len(df) >= n:
            df["high_low_range"] = df["high"].rolling(n).max() - df["low"].rolling(n).min()
            df["atr_sum"] = df["atr"].rolling(n).sum()
            # Avoid division by zero
            valid_range = df["high_low_range"] > 0
            df.loc[valid_range, "choppiness_index"] = 100 * np.log10(df.loc[valid_range, "atr_sum"] / 
                                                                    df.loc[valid_range, "high_low_range"]) / np.log10(n)
            # Clean up intermediate columns
            df.drop(columns=["high_low_range", "atr_sum"], inplace=True)
        
        # Consider very high choppiness index as an additional condition
        high_choppiness = False
        if "choppiness_index" in df.columns:
            high_choppiness = df["choppiness_index"] > 60  # Above 60 is considered choppy
        
        # Combine the indicators to detect choppy market - require more conditions to be met
        ...
        if high_choppiness is not False:
            choppy_conditions += high_choppiness.astype(int)
        
        # Mark as choppy if multiple conditions are met (using configurable threshold)
        df.loc[choppy_conditions >= self.chop_min_conditions, "is_choppy"] = 1
        ...
        df["ema_cross_count"] = df["ema_cross"].rolling(10).sum().fillna(0)
        
        # Only mark as choppy if there are MANY crosses AND other conditions also suggest choppiness
        crosses_and_other_conditions = (df["ema_cross_count"] >= 4) & (choppy_conditions >= 2)
        df.loc[crosses_and_other_conditions, "is_choppy"] = 1
        
        # Clean up intermediate columns
        df.drop(columns=["above_ema_short", "ema_cross", "ema_cross_count"], inplace=True)
        
        # Calculate a "relative trend strength" (RTS) metric to ignore choppy detection in strong trends
        df["relative_trend_strength"] = df["adx"] * df["trend_strength"]
        
        # If relative trend strength is high, override choppy market detection
        strong_trend = df["relative_trend_strength"] > (self.adx_threshold * 2)
        df.loc[strong_trend & (df["is_choppy"] == 1), "is_choppy"] = 0
        
        # Clean up other intermediate columns
        df.drop(columns=["ema_trend", "trend_strength", "relative_trend_strength"], inplace=True)
        if "choppiness_index" in df.columns:
            df.drop(columns=["choppiness_index"], inplace=True)
    ...
    def generate_signals(self, df, equity=10000.0):
        ...
        bullish_cross = (
            ...
            & (
                ...
                | (df["adx"] >= self.adx_threshold * 1.5)  # OR very strong trend
              )
        )
        ...
        # Exit conditions: MA crossover, RSI extreme, or STRONG evidence of choppy market
        # Only exit on choppy market if it has persisted for multiple bars
        exit_long = (
            (df["Position"].shift().fillna(0) == 1) 
            & (
                (df["fast_ma"] < df["slow_ma"]) 
                | (df["rsi"] > self.rsi_overbought) 
                | (df["is_choppy"].rolling(3).sum() >= 2)  # Require multiple choppy bars
              )
        )
